...
@Component({
  ...
  standalone: true,
  imports: [HeaderComponent, OptionsComponent, TableUsersComponent, NavegationComponent, CommonModule],
...
export class UsuariosComponent {
  ...
  // Elementos para el paginador
    ...
    cargarCuentas():void {
      ...
      this.apiService.users(this.paginaActual, this.option)
        .subscribe(respuesta => {
    ...
    Cuenta(datos: any) {
      this.apiService.createModifyUser(datos)
        .subscribe({
          next: (respuesta) => {
            if (respuesta.error) {
